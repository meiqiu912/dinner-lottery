...
高德 Web服务 Key 已以**分段编码**形式内置（源码无明文），所有用户打开"出去吃"即自动搜索周边 3 公里真实餐厅（评分/人均/地址/电话），**无需任何配置**。

- 内置 Key 为个人认证配额（搜索 5,000 次/月，坐标转换/逆地理 150,000 次/月），日常自用绰绰有余
- 开发者想换成自己的 Key：APP 首页 ⚙️ → 粘贴 → 保存（存 localStorage，优先于内置 Key）
- **边界说明**：编码内置防的是源码明文与仓库扫描，纯前端做不到绝对保密（浏览器网络面板可见请求参数）；要彻底隐藏需后端代理（如 Cloudflare Workers 转发，Key 存服务端环境变量）
